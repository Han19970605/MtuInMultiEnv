...
public:
    ...
    void InstallAllApplications(NodeContainer fromServers, NodeContainer destServers, double requestRate, struct cdf_table *cdfTable,
                                std::vector<Ipv4Address> destAddresses, uint32_t &flowCount, int port_start, int port_end, int packet_size,
                                double timesim_start, double timesim_end, double time_flow_launch_end, uint64_t bandwidth, double delay_prop);
    ...
    /**
         ...
         * change to install Normal and Install loss
        */
