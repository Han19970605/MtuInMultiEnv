...
int main(int argc, char *argv[])
{
    ...
    // fct filename
    std::string FCT_fileName = std::string("./data/FCT(wan)_").append(PROPOGATION_DELAY).append(std::string("_")).append(ES_BANDWIDTH).append(std::string("_"));
    FCT_fileName = FCT_fileName.append(std::to_string(LOSS_RATE)).append(std::string("_")).append(std::to_string(LOAD)).append(std::string(".csv"));
    ...
    Ipv4InterfaceContainer interface_e0s0 = ipv4Helper.Assign(devices_e0s0);
    Ipv4InterfaceContainer interface_e1s0 = ipv4Helper.Assign(devices_e1s0);
    Ipv4InterfaceContainer interface_e2s0 = ipv4Helper.Assign(devices_e2s0);
    Ipv4InterfaceContainer interface_e3s0 = ipv4Helper.Assign(devices_e3s0);
    ...
    Ipv4InterfaceContainer interface_s2e4 = ipv4Helper.Assign(devices_s2e4);
    Ipv4InterfaceContainer interface_s2e5 = ipv4Helper.Assign(devices_s2e5);
    Ipv4InterfaceContainer interface_s2e6 = ipv4Helper.Assign(devices_s2e6);
    Ipv4InterfaceContainer interface_s2e7 = ipv4Helper.Assign(devices_s2e7);
    ...
    dstAddress.push_back(interface_s2e4.GetAddress(1));
    dstAddress.push_back(interface_s2e5.GetAddress(1));
    dstAddress.push_back(interface_s2e6.GetAddress(1));
    dstAddress.push_back(interface_s2e7.GetAddress(1));
    ...
    ESNetHelper.InstallAllApplicationsInWAN(from_servers, dst_servers, request_rate, cdfTable, dstAddress,
                                            flowCount, PORT_START, PORT_END, START_TIME, END_TIME, END_TIME, bandwidth,
