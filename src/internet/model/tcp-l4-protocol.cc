...
#define NS_LOG_APPEND_CONTEXT                          \
                             ...
                             const uint8_t payload[8])
  {
    ...
    {
      NS_LOG_DEBUG("no endpoint found source=" << payloadSource << ", destination=" << payloadDestination << ", src=" << src << ", dst=" << dst);
    }
  }
                         ...
                         Ptr<Ipv4Interface> incomingInterface)
  {
    ...
    if (endPoints.empty())
    {
      ...
      NS_LOG_LOGIC("TcpL4Protocol " << this << " received a packet but"
                                    ...
                                    << " destination IP: " << incomingIpHeader.GetDestination() << " destination port: " << incomingTcpHeader.GetDestinationPort() << " source IP: " << incomingIpHeader.GetSource() << " source port: " << incomingTcpHeader.GetSourcePort());
    ...
    (*endPoints.begin())->ForwardUp(packet, incomingIpHeader, incomingTcpHeader.GetSourcePort(), incomingInterface);
  ...
  {
    NS_LOG_FUNCTION(this << packet << incomingIpHeader.GetSourceAddress() << incomingIpHeader.GetDestinationAddress());
    ...
    if (endPoints.empty())
    {
      NS_LOG_LOGIC("TcpL4Protocol " << this << " received a packet but"
                                               " no endpoints matched."
                                    << " destination IP: " << incomingIpHeader.GetDestinationAddress() << " destination port: " << incomingTcpHeader.GetDestinationPort() << " source IP: " << incomingIpHeader.GetSourceAddress() << " source port: " << incomingTcpHeader.GetSourcePort());
