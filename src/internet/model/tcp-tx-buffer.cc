...
namespace ns3
{
  ...
  TcpTxBuffer::GetTypeId(void)
  {
    static TypeId tid = TypeId("ns3::TcpTxBuffer")
                            ...
                            .AddTraceSource("BufferSize", "The current size in txbuffer",
                                            MakeTraceSourceAccessor(&TcpTxBuffer::m_size),
 ...
 */
  ...
  TcpTxBuffer::SizeFromSequence(const SequenceNumber32 &seq) const
  {
    ...
    NS_LOG_ERROR("Requested a sequence beyond our space (" << seq << " > " << lastSeq << "). Returning 0 for convenience.");
    return 0;
  }
  ...
  TcpTxBuffer::CopyFromSequence(uint32_t numBytes, const SequenceNumber32 &seq)
  {
    ...
    else if (m_firstByteSeq.Get().GetValue() + m_sentSize > seq.GetValue() && m_firstByteSeq.Get().GetValue() + m_sentSize < seq.GetValue() + s)
    {
  ...
  TcpTxBuffer::GetNewSegment(uint32_t numBytes)
  {
    ...
    NS_LOG_INFO("AppList start at " << startOfAppList << ", sentSize = " << m_sentSize << " firstByte: " << m_firstByteSeq);
    ...
    while (it != list.end())
    {
      ...
      if (seq < beginOfCurrentPacket + currentPacket->GetSize())
      {
        ...
        if (seq == beginOfCurrentPacket)
        {
          ...
          outItem = currentItem;
          NS_LOG_INFO("Current packet starts at seq " << seq << " ends at " << seq + currentPacket->GetSize());
        }
        else if (seq > beginOfCurrentPacket)
        {
          ...
          // start again.
          NS_LOG_INFO("we are at " << beginOfCurrentPacket << " searching for " << seq << " and now we recurse because packet ends at "
  ...
  TcpTxBuffer::DiscardUpTo(const SequenceNumber32 &seq)
  {
    ...
    NS_LOG_DEBUG("Remove up to " << seq << " lost: " << m_lostOut << " retrans: " << m_retrans << " sacked: " << m_sackedOut);
    ...
    while (m_size > 0 && offset > 0)
    {
      ...
      NS_ASSERT_MSG(item->m_startSeq == m_firstByteSeq,
                    "Item starts at " << item->m_startSeq << " while SND.UNA is " << m_firstByteSeq << " from " << *this);
      ...
      { // This packet is behind the seqnum. Remove this packet from the buffer
        ...
        NS_LOG_INFO("Removed " << *item << " lost: " << m_lostOut << " retrans: " << m_retrans << " sacked: " << m_sackedOut << ". Remaining data " << m_size);
        delete item;
      }
      else if (offset > 0)
      { // Part of the packet is behind the seqnum. Fragment
        ...
        NS_LOG_INFO("Fragmented one packet by size " << offset << ", new size=" << pktSize << " resulting item is " << *item << " status: " << *this);
        break;
      }
    }
    ...
    if (!m_sentList.empty())
    {
      ...
      NS_ASSERT_MSG(head->m_startSeq == seq,
                    "While removing up to " << seq << " we get SND.UNA to " << m_firstByteSeq << " this is the result: " << *this);
    }
    ...
    NS_LOG_DEBUG("Discarded up to " << seq << " lost: " << m_lostOut << " retrans: " << m_retrans << " sacked: " << m_sackedOut);
  ...
  TcpTxBuffer::Update(const TcpOptionSack::SackList &list)
  {
    ...
    for (auto option_it = list.begin(); option_it != list.end(); ++option_it)
    {
      ...
      while (item_it != m_sentList.end())
      {
        ...
        // here.
        if (beginOfCurrentPacket >= (*option_it).first && beginOfCurrentPacket + pktSize <= (*option_it).second)
        {
          if ((*item_it)->m_sacked)
          {
            NS_ASSERT(!(*item_it)->m_lost);
            NS_LOG_INFO("Received block " << *option_it << ", checking sentList for block " << *(*item_it) << ", found in the sackboard already sacked");
          }
          else
          {
            if ((*item_it)->m_lost)
            ...
            if (m_highestSack.first == m_sentList.end() || m_highestSack.second <= beginOfCurrentPacket + pktSize)
            {
              m_highestSack = std::make_pair(item_it, beginOfCurrentPacket);
            }

            NS_LOG_INFO("Received block " << *option_it << ", checking sentList for block " << *(*item_it) << ", found in the sackboard, sacking, current highSack: " << m_highestSack.second);
          }
          modified = true;
        }
        else if (beginOfCurrentPacket + pktSize > (*option_it).second)
        {
          ...
          NS_LOG_INFO("Received block [" << *option_it << ", checking sentList for block " << *(*item_it) << "], not found, breaking loop");
          break;
        }
  ...
  TcpTxBuffer::UpdateLostCount()
  {
    ...
    {
      NS_LOG_INFO("Status before the update: " << *this << ", will start from the latest sent item");
    }
    else
    {
      NS_LOG_INFO("Status before the update: " << *this << ", will start from item " << *(*m_highestSack.first));
    }
  ...
  TcpTxBuffer::BytesInFlight() const
  {
    NS_ASSERT_MSG(m_sackedOut + m_lostOut <= m_sentSize,
                  "Count of sacked " << m_sackedOut << " and lost " << m_lostOut << " is out of sync with sent list size " << m_sentSize << " " << *this);
  ...
  TcpTxBuffer::BytesInFlightRFC() const
  {
    ...
    NS_ASSERT_MSG(retrans == m_retrans, "Retrans Counted: " << retrans << " " << m_retrans << "\n"
                                                            << *this);
    NS_ASSERT_MSG(sackedOut == m_sackedOut, "Sacked counted: " << sackedOut << " " << m_sackedOut << *this);
  ...
  TcpTxBuffer::ConsistencyCheck() const
  {
    ...
    NS_ASSERT_MSG(sacked == m_sackedOut, "Counted SACK: " << sacked << " stored SACK: " << m_sackedOut);
    NS_ASSERT_MSG(lost == m_lostOut, " Counted lost: " << lost << " stored lost: " << m_lostOut);
    NS_ASSERT_MSG(retrans == m_retrans, " Counted retrans: " << retrans << " stored retrans: " << m_retrans);
  }
  ...
  operator<<(std::ostream &os, TcpTxBuffer const &tcpTxBuf)
  {
    ...
    os << "Sent list: " << ss.str() << ", size = " << tcpTxBuf.m_sentList.size() << " Total size: " << tcpTxBuf.m_size << " m_firstByteSeq = " << tcpTxBuf.m_firstByteSeq << " m_sentSize = " << tcpTxBuf.m_sentSize << " m_retransOut = " << tcpTxBuf.m_retrans << " m_lostOut = " << tcpTxBuf.m_lostOut << " m_sackedOut = " << tcpTxBuf.m_sackedOut;
