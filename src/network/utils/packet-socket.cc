...
namespace ns3
{
  ...
  int PacketSocket::DoBind(const PacketSocketAddress &address)
  {
    NS_LOG_FUNCTION(this << address);
  ...
  int PacketSocket::ShutdownSend(void)
  {
    NS_LOG_FUNCTION(this);
    if (m_state == STATE_CLOSED)
    {
  ...
  int PacketSocket::ShutdownRecv(void)
  {
    NS_LOG_FUNCTION(this);
    if (m_state == STATE_CLOSED)
    {
  ...
  int PacketSocket::Connect(const Address &ad)
  {
  ...
  int PacketSocket::Send(Ptr<Packet> p, uint32_t flags)
  {
  ...
  int PacketSocket::GetSockName(Address &address) const
  {
    NS_LOG_FUNCTION(this << address);
    PacketSocketAddress ad;
  ...
  int PacketSocket::GetPeerName(Address &address) const
  {
    NS_LOG_FUNCTION(this << address);
